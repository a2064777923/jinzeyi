...
function categoryLinks(category: MetaphysicsCategory): InternalLink[] {
  ...
  if (category === 'five-elements') {
    return [
      { href: '/tools/bazi', label: '八字五行', description: '查看出生四柱里的五行分布。', family: 'tool' },
  ...
  return [
    ...
    { href: '/tools/bazi', label: '八字排盘', description: '先从个人四柱理解术数语言。', family: 'tool' },
...
export const knowledgeIndexPage = {
  ...
  body: '命理知识库是今择易的术语和故事层。它不替代工具计算，也不把民俗语言包装成结论，而是把四柱、日主、十神、藏干、纳音、五行、宜忌、冲煞、吉时、二十八宿、紫微斗数和周天星斗放在同一个结构里，让用户在排盘、看黄历和择日时能快速理解每个词的来源、用法和边界。',
  faq: [
    {
      id: 'knowledge-tool',
    ...
    },
    {
      id: 'knowledge-source',
      question: '这些内容是断语吗？',
      answer: '不是。本站把命理、黄历和星宿内容作为传统文化与民俗参考，重要安排仍要结合现实条件和专业意见。',
    },
    {
      id: 'knowledge-order',
      question: '新手应该先看哪些词？',
      answer: '可先看四柱、日主、五行、宜忌和冲煞，再进入十神、藏干、纳音、二十八宿和紫微斗数等进阶内容。',
    },
  ],
  relatedLinks: [
    ...
    { href: '/jieri', label: '吉日查询', description: '把知识用于具体事项筛选。', family: 'jieri' },
  ],
  seed: {
    model: 'ContentPage',
    slug: 'knowledge',
...
export const metaphysicsEntries: MetaphysicsEntry[] = [
  buildEntry({
    ...
    detail: '干支是许多术数系统共同使用的时间语言。十天干偏向天象、节令和五行气机的表达，十二地支则连接月份、时辰、生肖和方位。看黄历或八字时，干支不是孤立符号，而是把日期、时辰和五行关系放到同一套编码里。',
    practicalUse: '在黄历页读干支时，先确认它对应年、月、日还是时；在八字页读干支时，再看它和日主、十神、藏干、五行分布的关系。',
  ...
  buildEntry({
    ...
    detail: '四柱把出生时间拆成四个层次：年柱看较大的时代和家族背景，月柱连接节令和成长环境，日柱是个人视角的中心，时柱常用于观察后续计划、子女和晚景等传统主题。今择易先把四柱排清楚，再把十神、藏干、纳音和五行强弱作为证据层展示。',
    practicalUse: '排盘时先核对四柱是否因出生地、真太阳时或时辰边界而变化，再读其他解释。择日时也可把候选日期的日柱与个人四柱分开比较。',
    ...
    chartHint: '专业盘面会把每柱的天干、地支、十神、藏干、纳音和十二长生状态并列展示。',
  ...
  buildEntry({
    ...
    detail: '日主也称日元，是八字里用来建立关系坐标的核心。其他天干、地支藏干和五行信号都会回到日主来比较：谁生我、我生谁、谁克我、我克谁、谁与我同类。今择易不会把日主写成性格定论，而是把它作为读盘的参照点。',
    practicalUse: '看八字结果时，先找到日主，再看十神、藏干和五行强弱为什么围绕它展开。若日主附近出现较多同类或生扶信息，页面会作为结构信号提示，而不是命运结论。',
    relatedTerms: ['four-pillars', 'ten-gods', 'hidden-stems', 'five-elements'],
    toolAppearances: ['八字排盘', '择日推荐', '命理知识提示'],
    mythologyStory: '如果四柱是一座庭院，日主就是站在院中的观察者。风从哪个方向来、哪一扇门更亮，都要先知道观察者站在哪里。',
    commonMisunderstandings: [
  ...
  buildEntry({
    ...
    detail: '十神不是神明名单，而是八字关系标签。它把五行生克和阴阳同异转换成可读角色，例如同类为比劫，我生者为食伤，我克者为财，克我者为官杀，生我者为印。专业盘面显示十神，是为了让用户知道每个干支相对日主扮演什么角色。',
    practicalUse: '读十神时先看它出现在哪一柱、是否透出天干、是否藏在地支，再结合五行强弱理解倾向。工具会把十神作为解释依据，不会把单个十神写成绝对好坏。',
    relatedTerms: ['day-master', 'hidden-stems', 'five-elements', 'four-pillars'],
    toolAppearances: ['八字排盘', '合婚参考', '择日推荐'],
    mythologyStory: '十神像一组剧中角色：朋友、表达、资源、规则、目标各有功能。一个角色上场不代表剧情结束，关键在角色之间怎样配合。',
    commonMisunderstandings: [
      '正财、偏财等名称不应直接理解成现实财富结果。',
      '官杀、印星等词有传统语境，阅读时应先看关系和位置，少做单字联想。',
    ],
    sourceNotes: [
      '参考子平八字十神关系体系。',
  ...
  buildEntry({
    ...
    detail: '地支不是单一五行标签，许多地支内部会藏一到三个天干。藏干让八字从表层四个天干扩展到更细的结构：有些力量透在天干，容易被看到；有些力量藏在地支，需要结合月令、透出和冲合来读。',
    practicalUse: '当五行表面看起来偏少时，先看地支藏干是否提供隐藏来源。今择易会给藏干做权重提示，让用户知道哪些是主气，哪些是中气或余气。',
    ...
    commonMisunderstandings: [
      '藏干不是可忽略的小字，它会影响五行分布和十神结构。',
      '藏干也不能简单相加成结论，主气、中气、余气的权重不同。',
    ],
    sourceNotes: [
      '参考地支藏干与月令主气的传统排盘规则。',
      '本站先用主次权重展示结构，不进入复杂门派差异比较。',
    ],
    chartHint: '专业盘面会把每个地支的藏干列出来，并显示它相对日主的十神关系。',
  ...
  buildEntry({
    ...
    practicalUse: '在今择易的八字盘里，纳音用于辅助理解每柱的文化意象。读盘时先看四柱、日主、十神、藏干和五行强弱，再把纳音作为补充故事线。',
    ...
    commonMisunderstandings: [
      '纳音不是八字结构的唯一核心，不能只看纳音五行判断强弱。',
      '纳音名称偏意象，和天干地支本身的五行读法要分开。',
    ],
    sourceNotes: [
      '参考六十甲子纳音体系和传统历书用语。',
      '本站将纳音作为文化意象字段，不把它单独用于高权重评分。',
    ],
    chartHint: '每柱都有对应纳音，页面会把它放在专业细节中，而不是放在首屏结论里。',
  ...
  buildEntry({
    ...
    detail: '五行不是五种物质的简单清单，而是一套关系模型。木火土金水之间有相生、相克和旺衰变化，可用于描述季节、方位、颜色、脏腑、姓名字义和八字结构。今择易会把五行当作解释工具，而不是把某个五行写成现实结果。',
    practicalUse: '在八字中，先区分可见天干地支和藏干权重；在姓名中，五行只作为字义与结构参考；在择日中，五行可帮助说明某日对某个日主的相对倾向。',
    relatedTerms: ['day-master', 'ten-gods', 'hidden-stems', 'na-yin'],
    toolAppearances: ['八字排盘', '姓名五行', '择日推荐'],
    mythologyStory: '五行常被想成四季流转：木生发，火舒展，土承载，金收敛，水收藏。它像一套观察变化的语言，而不是给人贴单色标签。',
    commonMisunderstandings: [
    ...
    starPersonalityMetaphor: '五行像调色盘，不是单色命名，而是看整幅画的比例、层次和光线。',
  ...
  buildEntry({
    ...
    detail: '黄历里的冲煞通常写成“冲某生肖，煞某方”。冲表示当日地支和某个地支相冲，常用于提醒相关生肖或事项要更谨慎；煞方则是传统择日里较不宜主动触动的方向。它是一层提醒，不是完整择日结果。',
    practicalUse: '涉及搬家、动土、安床、开业等事项时，先看冲煞是否直接触及本人或关键参与者生肖，再结合宜忌、值神、吉时和现实安排。',
    ...
    commonMisunderstandings: [
      ...
      '煞方不是现实安全判断，动工、出行和法律手续仍要按实际规则处理。',
    ],
    sourceNotes: [
      '参考传统黄历择日中地支相冲和煞方提示的用法。',
      '本站把冲煞作为评分和提醒维度之一，不单独给出最终结论。',
    ],
    chartHint: '择日推荐会把冲煞放进生肖/地支维度，并说明扣分原因。',
  ...
  buildEntry({
    ...
    detail: '宜忌是黄历最直观的事项列表，但它需要和具体场景对应。比如嫁娶、入宅、开市、动土、祭祀、出行等词都有传统语境。今择易会先匹配事项，再结合冲煞、时辰和场景权重给出解释。',
    practicalUse: '选日时先问“我要办的事在不在宜项里，是否被忌项命中”。若同一天有很多宜项，也要看目标事项是否匹配，而不是只看宜项数量。',
    ...
    commonMisunderstandings: [
      '宜项多不等于所有事情都合适，目标事项是否命中更重要。',
      '忌项命中时宜谨慎处理，大事可换日，小事可结合实际权衡。',
    ],
    sourceNotes: [
      '参考传统黄历对每日事项宜忌的分类。',
      '本站会把宜忌和场景规则对应，避免只按词条数量排序。',
    ],
    chartHint: '择日推荐会把事项匹配作为场景维度，并列出命中的宜项和需要留意的忌项。',
  ...
  buildEntry({
    ...
    category: 'almanac',
    name: '吉时',
    short: '一天十二个时辰各有吉凶和宜忌，吉时用于细化具体办事时间。',
    detail: '吉时把一天再分成十二段，每段有星神、宜忌和传统吉凶。它适合在已选日期后继续缩小时间范围。若整日基调较谨慎，吉时也只能作为局部参考，不能把日期整体改成完全适合。',
    practicalUse: '先选日期，再挑吉时。婚嫁、开业、搬家等场景可优先找事项相关的吉时；若只处理小事，吉时可以作为方便安排的提醒。',
    ...
    commonMisunderstandings: [
      '吉时不是万能开关，整日冲煞和忌项仍然需要看。',
      '同一个吉时也要结合地点、参与者时间和现实流程安排。',
    ],
    sourceNotes: [
      '参考黄历十二时辰、星神和时辰宜忌的常见结构。',
      '本站在评分中把吉时作为可用时段维度，而不是最终判断。',
    ],
    chartHint: '择日结果会优先显示可用吉时，并说明哪些时辰与目标事项更合拍。',
  ...
  buildEntry({
    ...
    detail: '二十八宿把天空分成东方青龙、北方玄武、西方白虎、南方朱雀四组，每组七宿。它既是古代天文星官系统的一部分，也进入历书、择日和民俗故事。今择易会把它作为星宿知识和未来择日细节的入口。',
    practicalUse: '在黄历或知识页看到星宿时，可先读它属于哪一方、哪一组，再看传统上偏向建设、整理、婚嫁、出行还是谨慎事项。它适合作为辅助层，而不是替代宜忌和冲煞。',
    ...
    commonMisunderstandings: [
      '二十八宿不是现代星座的中文翻译，它属于中国传统星官体系。',
      '星宿吉凶有历书语境，不能脱离事项类型单独使用。',
    ],
    sourceNotes: [
      '参考中国传统二十八宿与四象星官体系。',
      '本站先做文化解释和未来字段预留，深层星宿择日规则会逐步补充。',
    ],
  ...
  buildEntry({
    ...
    detail: '紫微斗数通常以出生时间排出十二宫位，再看紫微、天府、太阳、太阴等星曜及四化、庙旺陷等状态。它和八字都使用出生资料，但阅读语言不同：八字强调干支五行与十神，紫微斗数强调宫位、星曜组合和主题宫。',
    practicalUse: '今择易当前不排紫微命盘，但知识库先解释术语边界，为未来星曜、宫位和四化内容做准备。用户可先理解它和八字不是同一套模型。',
    ...
    commonMisunderstandings: [
      '紫微斗数不是八字的别名，两者盘面、术语和判断路径不同。',
      '星曜名称有神话色彩，但页面只会作为传统文化和工具解释使用。',
    ],
    sourceNotes: [
  ...
  buildEntry({
    ...
    detail: '周天星斗不是单个排盘字段，而是传统星空想象的总称。它把北斗、南斗、二十八宿、星官神名和民间故事放进一个更大的星空秩序里。今择易把它作为故事层入口，帮助用户理解为什么命理产品常把星、神、方位和时间放在一起。',
    ...
    commonMisunderstandings: [
      '周天星斗不是可直接计算个人结果的单一模块，它更像知识和故事总入口。',
