...
export const metaphysicsEntries: MetaphysicsEntry[] = [
  buildEntry({
    ...
    practicalTips: [
      ...
      '遇到"冲"或"煞"时，先找到是哪组地支相冲，再看是否涉及自己的生肖或相关事项。',
  ...
  buildEntry({
    ...
    paragraphs: [
      '四柱的排法有严格的历法规则。月柱不是按农历初一划分，而是以节气为界：立春到惊蛰为寅月，惊蛰到清明为卯月，以此类推。时柱则以子时（23:00-01:00）为起始，每两个小时一个时辰。关于子时的归属，传统上有"早子时"和"晚子时"的讨论：23:00到24:00出生的人，有的门派归入当日时柱，有的归入次日。今择易采用常见的排法供参考。',
    ...
    practicalTips: [
      ...
      '如果出生时间在时辰交界附近（如12:50或23:10），建议同时查看相邻两个时辰的盘面。',
      '四柱信息量很大，建议先看日主是什么天干，再逐步理解其他柱位与日主的关系。',
  ...
  buildEntry({
    ...
    paragraphs: [
      '日主的强弱判断是八字分析中最基础也最容易产生分歧的环节。传统方法首先看月令：日主在月支是否得令（即月支五行是否生扶日主）。其次看同类和生扶力量的多少：天干中是否有比肩、劫财，地支藏干中是否有印星和比劫。然后看克泄耗的力量：官杀、食伤和财星的强弱。这些力量的综合比较决定了日主的相对强弱。但需要注意，"强弱"只是一个参考维度，不能等同于好坏。',
    ...
    practicalTips: [
      ...
      '判断日主强弱时，先看月支（月令）是否支持日主，这是最重要的参考。',
  ...
  buildEntry({
    ...
    practicalTips: [
      '拿到排盘后，先看日柱天干（日主）是什么，再逐个理解其他天干相对日主的十神关系。',
      '天干上透出的十神和地支藏干中的十神要分开看，透出的信息更容易被观察到。',
      '不要因为某个十神名称听起来不好就担心，如"伤官"并不意味着一定会受伤。',
  ...
  buildEntry({
    ...
    practicalTips: [
      '在排盘中看到纳音时，先把它当作文化意象来理解，不要急于用它判断强弱。',
  ...
  buildEntry({
    ...
    paragraphs: [
      '五行的相生关系为：木生火、火生土、土生金、金生水、水生木。相克关系为：木克土、土克水、水克火、火克金、金克木。这两套关系是五行系统的基础逻辑。相生可以理解为滋养和促进：木材燃烧生火，火焰化灰成土，土中蕴藏矿金，金属冷凝生水（露珠），水灌溉滋养树木。相克可以理解为制约和平衡：树木扎根破土，土能挡水，水能灭火，火能熔金，金能伐木。这套关系不是简单的对抗，而是相互制约中的动态平衡。',
    ...
    practicalTips: [
      '看八字五行时，先数天干地支表面的五行分布，再把藏干计入，最后综合判断。',
      '五行"缺"某个元素不等于需要"补"，要看整体结构是否需要这个元素。',
      '起名参考五行时，字义和读音同样重要，不能只看偏旁部首。',
      '五行生克不是简单的"好"和"坏"，相生过多也可能失衡，相克有时反而是制约中的平衡。',
  ...
  buildEntry({
    ...
    paragraphs: [
      ...
      '冲煞在择日评分中的权重取决于事项类型。对于搬家、动土等涉及空间变动的事项，冲煞的权重通常较高；对于日常小事如购物、出行，冲煞的权重较低。今择易在择日推荐中会把冲煞作为扣分理由单独展示，帮助用户理解为什么某个日期被扣分。',
    ],
    practicalTips: [
      '看黄历时先确认"冲"的是哪个生肖，再看是否与自己或重要参与者的生肖相关。',
  ...
  buildEntry({
    ...
    paragraphs: [
      ...
      '吉时在今择易的择日推荐中属于细化维度。当用户确定了候选日期后，系统会展示当日各时辰的吉凶情况，帮助用户选择更合适的办事时间。但吉时只是参考，实际安排还要考虑参与者的时间、交通状况和具体场景。',
    ],
    practicalTips: [
  ...
  buildEntry({
    ...
    paragraphs: [
      ...
      '周天星斗的概念对理解中国传统术数的整体框架非常重要。无论是八字、紫微斗数、风水还是奇门遁甲，都或多或少涉及星曜、方位和时间的配合。了解周天星斗的文化背景，有助于理解这些术数系统为何会把"天上的秩序"和"人间的安排"联系在一起。',
    ],
    practicalTips: [
  ...
  buildEntry({
    ...
    practicalTips: [
      '拿到排盘后，先看四柱天干是什么，再看日柱天干（日主）与其余天干的十神关系。',
  ...
  buildEntry({
    ...
    paragraphs: [
      ...
      '地支六冲（子午冲、丑未冲、寅申冲、卯酉冲、辰戌冲、巳亥冲）和六合一样，是地支之间最基本的关系。冲意味着对立和冲突，五行属性相克。但冲不一定是坏事，冲去忌神可能有利，冲去喜神则可能不利。《滴天髓》强调要看冲的具体对象和对日主的影响。',
