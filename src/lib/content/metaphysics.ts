...
export const knowledgeIndexPage = {
  ...
  body: '命理知识库是今择易的术语和故事层。它不替代工具计算，也不把民俗语言包装成结论，而是把四柱、日主、十神、藏干、纳音、五行、宜忌、冲煞、吉时、二十八宿、紫微斗数和周天星斗放在同一个结构里，方便排盘、看黄历和择日时快速理解每个词的来源、用法和边界。',
  faq: [
    {
      id: 'knowledge-tool',
...
export const metaphysicsEntries: MetaphysicsEntry[] = [
  ...
  buildEntry({
    ...
    detail: '日主也称日元，是八字里用来建立关系坐标的核心。其他天干、地支藏干和五行信号都会回到日主来比较：谁生我、我生谁、谁克我、我克谁、谁与我同类。日主是读盘参照点，不是性格定论。',
    practicalUse: '看八字结果时，日主是中心点；十神、藏干和五行强弱都围绕它展开。日主附近若有较多同类或生扶信息，适合当作结构信号，不是命运结论。',
  ...
  buildEntry({
    ...
    detail: '十神不是神明名单，而是八字关系标签。它把五行生克和阴阳同异转换成可读角色，例如同类为比劫，我生者为食伤，我克者为财，克我者为官杀，生我者为印。专业盘面显示十神，能看出每个干支相对日主扮演什么角色。',
  ...
  buildEntry({
    ...
    detail: '五行不是五种物质的简单清单，而是一套关系模型。木火土金水之间有相生、相克和旺衰变化，可描述季节、方位、颜色、脏腑、姓名字义和八字结构。五行适合作为解释工具，不适合把某个五行写成现实结果。',
  ...
  buildEntry({
    ...
    detail: '黄历里的冲煞通常写成“冲某生肖，煞某方”。冲表示当日地支和某个地支相冲，常提醒相关生肖或事项要更谨慎；煞方则是传统择日里较不宜主动触动的方向。它是一层提醒，不是完整择日结果。',
  ...
  buildEntry({
    ...
    practicalUse: '选日时，关键问题是“我要办的事在不在宜项里，是否被忌项命中”。若同一天有很多宜项，也要看这件事是否匹配，而不是只看宜项数量。',
  ...
  buildEntry({
    ...
    practicalUse: '在黄历或知识页看到星宿时，方位、分组和事项倾向都值得留意。它适合作为辅助层，而不是替代宜忌和冲煞。',
  ...
  buildEntry({
    ...
    practicalUse: '今择易当前不排紫微命盘，知识库只解释术语边界，为未来星曜、宫位和四化内容做准备。紫微斗数和八字不是同一套模型。',
  ...
  buildEntry({
    ...
    detail: '周天星斗不是单个排盘字段，而是传统星空想象的总称。它把北斗、南斗、二十八宿、星官神名和民间故事放进一个更大的星空秩序里。今择易把它作为故事层入口，用来说明为什么命理产品常把星、神、方位和时间放在一起。',
