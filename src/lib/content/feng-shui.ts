...
const articleSeeds: Array<{
  ...
  },
  {
    ...
    sections: [
      { heading: '灶位要稳，水火要分', body: '《阳宅三要》把灶列为宅之三要之一，核心不是灶的材质，而是灶在厨房中的位置和朝向。落到现代厨房，灶台背后应靠实墙，避免悬空或正对窗户风口，否则明火受风不稳，日常做饭也不安心。水槽与灶台不宜紧贴或正对面——中间隔一段台面，既符合操作分区逻辑，也避免传统所说的水火相冲。冰箱属水，也不宜紧挨灶台。实际操作中，灶台与水槽之间保留至少60厘米的台面，就是最实用的缓冲。灶台上方不宜正对横梁或吊柜尖角下压，若无法改动，可调整灶位或做圆角处理。' },
