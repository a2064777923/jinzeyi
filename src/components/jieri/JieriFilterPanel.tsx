    ...
    <aside className="flex flex-col gap-4 rounded-lg border border-border bg-card p-4">
      ...
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
        <Button
          ...
          size="icon"
          nativeButton={false}
          render={<Link href={`/jieri/${scene.slug}/${prevYear}`} />}
          aria-label={localizeBodyCopy(locale, `打开${prevYear}年${scene.name}吉日`)}
        >
          <ChevronLeft />
        </Button>
        ...
        <Button
          ...
          size="icon"
          nativeButton={false}
          render={<Link href={`/jieri/${scene.slug}/${nextYear}`} />}
          aria-label={localizeBodyCopy(locale, `打开${nextYear}年${scene.name}吉日`)}
        >
          <ChevronRight />
        </Button>
      </div>
