...
export function BaziResult({ result }: { result: BaziResultData }) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-semibold">排盘结果</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            {result.city.name} · {result.genderLabel} · {result.trueSolarTime.adjusted}
          </p>
        </div>
        <Badge variant="outline">{result.trueSolarTime.offsetMinutes >= 0 ? '+' : ''}{result.trueSolarTime.offsetMinutes} 分钟</Badge>
      </div>
          ...
          return (
            <div key={key} className="min-h-28 rounded-lg border border-border bg-background p-3 text-center">
              ...
              <p className="mt-3 text-xs leading-5 text-muted-foreground">
                天干{pillar.heavenlyStem}属{pillar.stemElement} · 地支{pillar.earthlyBranch}属{pillar.branchElement}
              </p>
            </div>
          );
        })}
      ...
      <div className="rounded-lg border border-border bg-background p-3">
        <h3 className="text-sm font-semibold">五行分布</h3>
